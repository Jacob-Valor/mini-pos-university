...
public sealed class SalesRepositoryIntegrationTests
{
    ...
    [Fact]
    public async Task CreateSaleAsync_WhenQuantityExceedsStock_RollsBackTransaction()
    {
        var productRepo = new ProductRepository(_fixture.ConnectionFactory);
        var salesRepo = new SalesRepository(_fixture.ConnectionFactory);

        var barcode = ("Q" + Guid.NewGuid().ToString("N"))[..13];
        ...
        var unchanged = await productRepo.GetProductByBarcodeAsync(barcode);
        Assert.NotNull(unchanged);
        Assert.Equal(1, unchanged!.Quantity);
        Assert.Equal(0, await CountSalesByDateAndSubtotalAsync(dateSale, subTotal));
    }

    [Fact]
    public async Task CreateSaleAsync_WhenDetailsAreEmpty_DoesNotInsertSale()
    {
        var salesRepo = new SalesRepository(_fixture.ConnectionFactory);

        var now = DateTime.UtcNow;
        var dateSale = new DateTime(now.Year, now.Month, now.Day, now.Hour, now.Minute, now.Second, DateTimeKind.Utc);
        var subTotal = 4321.10m;
        ...
        Assert.False(await salesRepo.CreateSaleAsync(sale, Array.Empty<SaleDetail>()));
        Assert.Equal(0, await CountSalesByDateAndSubtotalAsync(dateSale, subTotal));
    }

    private async Task<int> CountSalesByDateAndSubtotalAsync(DateTime dateSale, decimal subTotal)
    {
        await using var connection = await _fixture.ConnectionFactory.OpenConnectionAsync();
        await using var cmd = new MySqlCommand("SELECT COUNT(*) FROM sales WHERE date_sale = @date AND subtotal = @sub", connection);
        ...
        cmd.Parameters.Add(new MySqlParameter("@sub", MySqlDbType.Decimal)
        {
            Precision = 12,
            Scale = 2,
            Value = subTotal
