...
public sealed class SalesRepository : ISalesRepository
{
    ...
    public async Task<bool> CreateSaleAsync(Sale sale, IEnumerable<SaleDetail> details)
    {
        ...
        {
            Log.Warning("Sale creation rejected because there are no sale details.");
            return false;
        }
        ...
        try
        {
            ...
            foreach (var item in detailList)
            {
                ...
                var insertedRows = await detailCmd.ExecuteNonQueryAsync();
                if (insertedRows != 1)
                    throw new InvalidOperationException("Failed to insert a sale detail row.");
                ...
                var updatedRows = await stockCmd.ExecuteNonQueryAsync();
                if (updatedRows != 1)
                    throw new InvalidOperationException($"Insufficient stock or missing product for '{item.ProductId}'.");
            }
        ...
        catch (Exception ex)
        {
            ...
            Log.Error(ex, "Error creating sale for customer {CustomerId}", sale.CustomerId);
            return false;
        }
    }
    ...
    public async Task<List<SalesReportItem>> GetSalesReportAsync(DateTime startDate, DateTime endDate)
    {
        ...
        try
        {
            ...
            command.Parameters.Add(new MySqlParameter("@endExclusive", MySqlDbType.DateTime)
            {
                Value = endDate.Date.AddDays(1)
            });
